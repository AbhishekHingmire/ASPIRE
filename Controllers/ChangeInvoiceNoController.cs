...
{
    public class ChangeInvoiceNoController : Controller
    {
        private readonly IChangeInvoiceNo _changeInvoiceNoRepository;
        private readonly ILogger<ChangeInvoiceNoController> _logger;
        private readonly IConfiguration _configuration;
        private readonly IWebHostEnvironment _env;
        private readonly ISettings _settingRepository;

        public ChangeInvoiceNoController(IChangeInvoiceNo changeInvoiceNoRepository, ILogger<ChangeInvoiceNoController> logger, IConfiguration configuration, IWebHostEnvironment webHostEnvironment , ISettings settingRepository)
        {
            _changeInvoiceNoRepository = changeInvoiceNoRepository;
            _logger = logger;
        ...
        [HttpPost]
        [Authorize(Roles = "SuperAdmin")]
        public async Task<IActionResult> BulkChangeInvoiceNo(IFormFile file)
        {
            ...
            var transactions = new List<ChangeInvoiceNoModel>();
            ...
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    using (var workbook = new XLWorkbook(stream))
                    {
                        ...
                        {
                            var transaction = new ChangeInvoiceNoModel
                            {
                                AdminUserID = Convert.ToInt64(userId),
                                PartnerID = Convert.ToInt64(partnerIdCookie),
                                LoanAppNo = worksheet.Cell(row, 1).GetValue<string>(),
                                OldInvoiceNo = worksheet.Cell(row, 2).GetValue<string>(),
                                NewInvoiceNo = worksheet.Cell(row, 3).GetValue<string>(),
                            };
                ...
                var failedTransactions = await _changeInvoiceNoRepository.ChangeInvoice(transactions);
        ...
        private async Task<string> CreateErrorExcelFile(IFormFile file, List<ChangeInvoiceNoModel> failedTransactions)
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                worksheet.Cell(1, 2).Value = "OldInvoiceNo";
                worksheet.Cell(1, 3).Value = "NewInvoiceNo";
                ...
                for (int i = 0; i < failedTransactions.Count; i++)
                {
                    ...
                    worksheet.Cell(i + 2, 2).Value = transaction.OldInvoiceNo;
                    worksheet.Cell(i + 2, 3).Value = transaction.NewInvoiceNo;
                }
