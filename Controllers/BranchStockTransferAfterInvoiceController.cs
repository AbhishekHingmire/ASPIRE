...
{
    public class BranchStockTransferAfterInvoiceController : Controller
    {
        private readonly IBranchStockTransferAfterInvoice _branchStockTransferAfterInvoiceRepository;
        private readonly ILogger<BranchStockTransferAfterInvoiceController> _logger;
        private readonly IConfiguration _configuration;
        private readonly IWebHostEnvironment _env;
        private readonly ISettings _settingRepository;

        public BranchStockTransferAfterInvoiceController(IBranchStockTransferAfterInvoice branchStockTransferAfterInvoiceRepository, ILogger<BranchStockTransferAfterInvoiceController> logger, IConfiguration configuration, IWebHostEnvironment webHostEnvironment, ISettings settingRepository)
        {
            _branchStockTransferAfterInvoiceRepository = branchStockTransferAfterInvoiceRepository;
            _logger = logger;
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                var screenRight = UserHelper.GetScreenAndRight(User, EnumScreenNames.BranchStockTransferAfterInvoice);
                if (screenRight != null)
                {
        ...
        [HttpPost]
        [Authorize(Roles = "Administrator, NormalUser")]
        public async Task<IActionResult> BranchStockTransferInvoice(IFormFile file)
        {
            ...
            var transactions = new List<BranchStockTransferAfterInvoiceModel>();
            ...
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    using (var workbook = new XLWorkbook(stream))
                    {
                        ...
                        {
                            var transaction = new BranchStockTransferAfterInvoiceModel
                            {
                                ...
                                ToBranchCode = worksheet.Cell(row, 2).GetValue<string>(),
                            };
                ...
                var failedTransactions = await _branchStockTransferAfterInvoiceRepository.BranchStockTransfer(transactions);
        ...
        private async Task<string> CreateErrorExcelFile(IFormFile file, List<BranchStockTransferAfterInvoiceModel> failedTransactions)
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                worksheet.Cell(1, 2).Value = "ToBranchCode";
                ...
                for (int i = 0; i < failedTransactions.Count; i++)
                {
                    ...
                    worksheet.Cell(i + 2, 2).Value = transaction.ToBranchCode;
                }
