...
{
    public class DeleteSalesOrderController : Controller
    {
        private readonly IDeleteSalesOrder _deleteSalesOrderRepository;
        private readonly ILogger<DeleteSalesOrderController> _logger;
        private readonly IConfiguration _configuration;
        private readonly IWebHostEnvironment _env;
        private readonly ISettings _settingRepository;

        public DeleteSalesOrderController(IDeleteSalesOrder deleteSalesOrderRepository, ILogger<DeleteSalesOrderController> logger, IConfiguration configuration, IWebHostEnvironment webHostEnvironment, ISettings settingRepository)
        {
            _deleteSalesOrderRepository = deleteSalesOrderRepository;
            _logger = logger;
        ...
        [HttpPost]
        [Authorize(Roles = "SuperAdmin")]
        public async Task<IActionResult> BulkDeleteSO(IFormFile file)
        {
            ...
            var transactions = new List<DeleteSalesOrderModel>();
            ...
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    using (var workbook = new XLWorkbook(stream))
                    {
                        ...
                        {
                            var transaction = new DeleteSalesOrderModel
                            {
                                ...
                                Remark = worksheet.Cell(row, 2).GetValue<string>(),
                            };
                ...
                var failedTransactions = await _deleteSalesOrderRepository.BulkDeleteSalesOrder(transactions);
        ...
        private async Task<string> CreateErrorExcelFile(IFormFile file, List<DeleteSalesOrderModel> failedTransactions)
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                for (int i = 0; i < failedTransactions.Count; i++)
                {
                    ...
                    worksheet.Cell(i + 2, 2).Value = transaction.Remark;
                }
