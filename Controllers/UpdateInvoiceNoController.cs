...
{
    public class UpdateInvoiceNoController : Controller
    {
        private readonly IUpdateInvoiceNo _updateInvoiceNoRepository;
        private readonly ILogger<UpdateInvoiceNoController> _logger;
        private readonly IConfiguration _configuration;
        private readonly IWebHostEnvironment _env;
        private readonly ISettings _settingRepository;

        public UpdateInvoiceNoController(IUpdateInvoiceNo updateInvoiceNoRepository, ILogger<UpdateInvoiceNoController> logger, IConfiguration configuration, IWebHostEnvironment webHostEnvironment, ISettings settingRepository)
        {
            _updateInvoiceNoRepository = updateInvoiceNoRepository;
            _logger = logger;
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                var screenRight = UserHelper.GetScreenAndRight(User, EnumScreenNames.UpdateInvoiceNo);
                if (screenRight != null)
                {
        ...
        public async Task<IActionResult> UploadInvoice(IFormFile file)
        {
            ...
            var transactions = new List<UpdateInvoiceNoModel>();
            ...
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    using (var workbook = new XLWorkbook(stream))
                    {
                        ...
                        {
                            var transaction = new UpdateInvoiceNoModel
                            {
                                ...
                                DocketNo = worksheet.Cell(row, 3).GetValue<string>(),
                            };
                ...
                var failedTransactions = await _updateInvoiceNoRepository.BulkUpdateInvoiceNo(transactions);
        ...
        private async Task<string> CreateErrorExcelFile(IFormFile file, List<UpdateInvoiceNoModel> failedTransactions)
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                worksheet.Cell(1, 3).Value = "DocketNo";
                ...
                for (int i = 0; i < failedTransactions.Count; i++)
                {
                    ...
                    worksheet.Cell(i + 2, 3).Value = transaction.DocketNo;
                }
