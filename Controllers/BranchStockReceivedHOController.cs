...
{
    public class BranchStockReceivedHOController : Controller
    {
        private readonly IBranchComplaintHO _branchComplaintHORepository;
        private readonly IBranchStock _branchStockRepository;
        private readonly ILogger<BranchStockReceivedHOController> _logger;
        ...
        public BranchStockReceivedHOController(IBranchStock branchStockRepository, ILogger<BranchStockReceivedHOController> logger, IConfiguration configuration, IWebHostEnvironment webHostEnvironment, ISettings settingRepository, ApplicationDbContext context, IImportSalesOrder importSalesOrderRepository, IBranchComplaintHO branchComplaintHORepository)
        {
        ...
        public void ExportSalesReport(string RegionCode, long ItemID, int DisbStatus)
        {
            try
            {
                var name = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Name)?.Value;
                ...
                {
                    name, RegionCode, ItemID, DisbStatus
                });
            }
            catch (Exception ex)
            {
