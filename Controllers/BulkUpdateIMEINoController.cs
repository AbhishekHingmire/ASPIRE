...
{
    public class BulkUpdateIMEINoController : Controller
    {
        private readonly IBulkUpdateIMEINo _bulkUpdateIMEINoRepository;
        ...
        public BulkUpdateIMEINoController(IBulkUpdateIMEINo bulkUpdateIMEINoRepository, ILogger<BookedOrdersToSOController> logger, IConfiguration configuration, IWebHostEnvironment webHostEnvironment, ISettings settingRepository)
        {
            _bulkUpdateIMEINoRepository = bulkUpdateIMEINoRepository;
            _logger = logger;
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                var screenRight = UserHelper.GetScreenAndRight(User, EnumScreenNames.BulkUploadIMEINo);
                if (screenRight != null)
                {
        ...
        [HttpPost]
        [Authorize(Roles = "Administrator, NormalUser")]
        public async Task<IActionResult> BulkUploadIMEI(IFormFile file)
        {
            ...
            var transactions = new List<BulkUpdateIMEINoModel>();

            if (transactions.Count == 0)
            {
                return BadRequest("No records found in file");
            }
            ...
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    using (var workbook = new XLWorkbook(stream))
                    {
                        ...
                        {
                            var transaction = new BulkUpdateIMEINoModel
                            {
                                ...
                                IMEI_No = worksheet.Cell(row, 3).GetValue<string>(),
                                DocketNo = worksheet.Cell(row, 4).GetValue<string>(),
                                InvoiceDate = worksheet.Cell(row, 5).GetValue<string>(),
                            };
                ...
                var failedTransactions = await _bulkUpdateIMEINoRepository.UpdateIMEINo(transactions);
        ...
        private async Task<string> CreateErrorExcelFile(IFormFile file, List<BulkUpdateIMEINoModel> failedTransactions)
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                worksheet.Cell(1, 3).Value = "IMEI_No";
                worksheet.Cell(1, 4).Value = "DocketNo";
                worksheet.Cell(1, 5).Value = "InvoiceDate";
                ...
                for (int i = 0; i < failedTransactions.Count; i++)
                {
                    ...
                    worksheet.Cell(i + 2, 3).Value = transaction.IMEI_No;
                    worksheet.Cell(i + 2, 4).Value = transaction.DocketNo;
                    worksheet.Cell(i + 2, 5).Value = transaction.InvoiceDate;
                }
