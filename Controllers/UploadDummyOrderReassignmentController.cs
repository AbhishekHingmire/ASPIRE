...
{
    public class UploadDummyOrderReassignmentController : Controller
    {
        private readonly IUploadDummyOrderReassignment _uploadDummyOrderReassignmentRepository;
        private readonly ILogger<UploadDummyOrderReassignmentController> _logger;
        private readonly IConfiguration _configuration;
        private readonly IWebHostEnvironment _env;
        private readonly ISettings _settingRepository;


        public UploadDummyOrderReassignmentController(IUploadDummyOrderReassignment uploadDummyOrderReassignmentRepository, ILogger<UploadDummyOrderReassignmentController> logger, IConfiguration configuration, IWebHostEnvironment webHostEnvironment, ISettings settingRepository)
        {
            _uploadDummyOrderReassignmentRepository = uploadDummyOrderReassignmentRepository;
            _logger = logger;
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                var screenRight = UserHelper.GetScreenAndRight(User, EnumScreenNames.UploadDummyOrderReassignment);
                if (screenRight != null)
                {
        ...
        [HttpPost]
        [Authorize(Roles = "Administrator, NormalUser")]
        public async Task<IActionResult> DummyOrderReassignment(IFormFile file)
        {
            ...
            var transactions = new List<UploadDummyOrderReassignmentModel>();
            ...
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    using (var workbook = new XLWorkbook(stream))
                    {
                        ...
                        {
                            var transaction = new UploadDummyOrderReassignmentModel
                            {
                                AdminUserID = Convert.ToInt64(userId),
                                OldLoanAppNo = worksheet.Cell(row, 1).GetValue<string>(),
                                NewLoanAppNo = worksheet.Cell(row, 2).GetValue<string>(),
                                CustomerName = worksheet.Cell(row, 3).GetValue<string>(),
                            };
                ...
                var failedTransactions = await _uploadDummyOrderReassignmentRepository.UploadDummyOrder(transactions);
        ...
        private async Task<string> CreateErrorExcelFile(IFormFile file, List<UploadDummyOrderReassignmentModel> failedTransactions)
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                worksheet.Cell(1, 1).Value = "OldLoanAppNo";
                worksheet.Cell(1, 2).Value = "NewLoanAppNo";
                worksheet.Cell(1, 3).Value = "CustomerName";
                worksheet.Cell(1, 4).Value = "Reason";

                for (int i = 0; i < failedTransactions.Count; i++)
                {
                    var transaction = failedTransactions[i];
                    worksheet.Cell(i + 2, 1).Value = transaction.OldLoanAppNo;
                    worksheet.Cell(i + 2, 2).Value = transaction.NewLoanAppNo;
                    worksheet.Cell(i + 2, 3).Value = transaction.CustomerName;
                    worksheet.Cell(i + 2, 4).Value = transaction.Reason;
                }
