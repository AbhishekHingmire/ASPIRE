...
namespace IMS2.Controllers
{
    ...
    {
        private readonly IUpdateOrderStatus _UpdateOrderStatusRepository;
        ...
        public UpdateOrderStatusController(IUpdateOrderStatus UpdateOrderStatusRepository, ILogger<UpdateOrderStatusController> logger, IConfiguration configuration, IWebHostEnvironment webHostEnvironment, ISettings settingRepository)
        {
            _UpdateOrderStatusRepository = UpdateOrderStatusRepository;
            _logger = logger;
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                var screenRight = UserHelper.GetScreenAndRight(User, EnumScreenNames.UpdateOrderStatus);
                if (screenRight != null)
                {
        ...
        [HttpPost]
        [Authorize(Roles = "Administrator, NormalUser")]
        public async Task<IActionResult> UploadOrderStatus(IFormFile file)
        {
            ...
            var transactions = new List<UpdateOrderStatusModel>();
            ...
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    using (var workbook = new XLWorkbook(stream))
                    {
                        ...
                        {
                            var transaction = new UpdateOrderStatusModel
                            {
                                AdminUserID = Convert.ToInt64(userId),
                                PartnerID = Convert.ToInt64(partnerIdCookie),
                                AppNo = worksheet.Cell(row, 1).GetValue<string>(),
                                StatusCode = worksheet.Cell(row, 2).GetValue<string>(),
                                Remarks = worksheet.Cell(row, 3).GetValue<string>(),
                            };
                ...
                var failedTransactions = await _UpdateOrderStatusRepository.AddOrderStatusAsync(transactions);
        ...
        private async Task<string> CreateErrorExcelFile(IFormFile file, List<FailedTransactionModel> failedTransactions)
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                worksheet.Cell(1, 2).Value = "StatusCode";
